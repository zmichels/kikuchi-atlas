...
INTENSITY_LEDGER_SCHEMA = "kikuchi.ice-intensity-ledger/v2"
    ...
    def identity_dict(self) -> dict[str, object]:
        return {
            ...
            "geometry": {
                ...
                "maximum_relief_mm": self.maximum_relief_mm,
                "topology": "icosphere",
                "subdivisions": self.subdivisions,
                "direction": "bright_outward",
            },
            "mapping": {
                "percentiles": {"lower": self.lower_percentile, "upper": self.upper_percentile},
                "gamma": self.gamma,
            },
        }
...
def _recipe(path: str | Path) -> IceIntensityRecipe:
    ...
    try:
        if (
            set(raw) != {"schema", "geometry", "mapping"}
            or raw["schema"] != "kikuchi.ice-intensity-globe-recipe/v1"
        ):
            raise ValueError
        geometry, mapping = raw["geometry"], raw["mapping"]
        if (
            set(geometry)
            != {"base_diameter_mm", "maximum_relief_mm", "topology", "subdivisions", "direction"}
            or set(mapping) != {"percentiles", "gamma"}
            or set(mapping["percentiles"]) != {"lower", "upper"}
        ):
            raise ValueError
        diameter, relief = float(geometry["base_diameter_mm"]), float(geometry["maximum_relief_mm"])
        lower, upper, gamma = (
            float(mapping["percentiles"]["lower"]),
            float(mapping["percentiles"]["upper"]),
            float(mapping["gamma"]),
        )
        subdivisions = geometry["subdivisions"]
        if (
            geometry["topology"] != "icosphere"
            or geometry["direction"] != "bright_outward"
            or type(subdivisions) is not int
            or not 0 <= subdivisions <= 7
            or not np.isfinite((diameter, relief, lower, upper, gamma)).all()
            or diameter <= 0
            or relief <= 0
    ...
    return IceIntensityRecipe(
        ...
        upper,
        gamma,
        stable_id("ice-intensity-globe-recipe", identity),
    )
...
def _write_json(path: Path, payload: object) -> None:
    path.write_text(
        json.dumps(plain_data(payload), indent=2, sort_keys=True, allow_nan=False) + "\n",
        encoding="utf-8",
    )
...
def _preview(path: Path, geometry) -> None:
    ...
    rgba[:, :3] *= (0.35 + 0.65 * np.clip(normals @ np.array((0.35, -0.45, 0.82)), 0.0, 1.0))[
        :, None
    ]
...
def _result(build_id: str, path: Path) -> IceIntensityGlobeBuildResult:
    return IceIntensityGlobeBuildResult(
        build_id,
        path,
        path / "intensity-globe-manifest.json",
        path / "ice-ih-intensity-relief.stl",
        path / "ice-ih-intensity-relief-preview.png",
        path / "intensity-field.npz",
        path / "intensity-ledger.json",
        path / "mesh-validation.json",
    )


def _simulation_provenance(
    simulation: KinematicalSimulation, field: IceIntensityField
) -> dict[str, object]:
    """Return plain, standalone kinematical source-to-field evidence."""
    master = simulation.master_stereographic
    metadata = plain_data(master.metadata)
    projection_ledger = plain_data(simulation.projection_ledger)
    return {
        "master": {
            "master_product_id": master.product_id,
            "master_array_sha256": master.array_sha256,
            "array_dtype": master.intensity.dtype.str,
            "array_shape": list(master.intensity.shape),
            "metadata": metadata,
        },
        "source_structure": {
            "structure_id": metadata.get("source_structure_id"),
            "source_id": metadata.get("source_id"),
            "checksum_sha256": metadata.get("source_sha256"),
            "source_setting": metadata.get("source_setting"),
            "simulation_setting": metadata.get("simulation_setting"),
        },
        "kinematical_recipe_id": metadata.get("recipe_id"),
        "projection_ledger": projection_ledger,
        "reflector_catalog_evidence": plain_data(simulation.reflector_catalog),
        "sampling": {
            "contract": STEREOGRAPHIC_SAMPLING_CONTRACT,
            "valid_disk_domain": "X^2 + Y^2 <= 1",
            "equator_domain": "X^2 + Y^2 == 1",
            "hemisphere_order": ["upper", "lower"],
            "equator_owner": field.seam.equator_owner,
            "disk_domain_sample_count": len(field.raw_values),
        },
        "seam_diagnostics": asdict(field.seam),
    }
...
def _validate_serialized_stl(payload: bytes, source, geometry):
    ...
    upper_bound = (
        geometry.base_radius_mm + geometry.maximum_relief_mm + _STL_RADIAL_SAFETY_MARGIN_MM
    )
...
def build_ice_intensity_globe(
    kinematical_recipe: str | Path, globe_recipe: str | Path, output_root: str | Path
) -> IceIntensityGlobeBuildResult:
    ...
    lower, upper = np.percentile(
        field.raw_values, (recipe.lower_percentile, recipe.upper_percentile)
    )
    ...
    sampled_raw = sample_stereographic_grid(
        field, topology.directions, upper_grid=field.upper_grid, lower_grid=field.lower_grid
    )
    mapped = sample_stereographic_grid(
        field, topology.directions, upper_grid=mapped_upper, lower_grid=mapped_lower
    )
    mapped = _serialization_safe_values(mapped, recipe.maximum_relief_mm)
    geometry = build_radial_geometry(
        topology,
        mapped,
        GlobeGeometrySpec(recipe.base_diameter_mm, recipe.maximum_relief_mm, recipe.subdivisions),
    )
    ...
    provenance = _simulation_provenance(simulation, field)
    identity = {
        "schema": INTENSITY_BUILD_SCHEMA,
        "ledger_schema": INTENSITY_LEDGER_SCHEMA,
        "product_kind": "intensity_relief",
        "recipe": recipe.identity_dict(),
        "recipe_id": recipe.recipe_id,
        "kinematical_recipe_id": simulation.master_stereographic.metadata["recipe_id"],
        "master_product_id": field.master_product_id,
        "master_array_sha256": field.master_array_sha256,
    ...
    if completed.exists() or partial.exists():
        raise FileExistsError(
            f"Ice intensity globe destination already exists: {completed if completed.exists() else partial}"
        )
    partial.mkdir()
    try:
        ...
        arrays = {
            ...
            "sampled_raw": np.asarray(sampled_raw, dtype="<f8"),
            "mapped_values": np.asarray(mapped, dtype="<f8"),
        ...
        _write_json(
            partial / result.ledger.name,
            {
                "schema": INTENSITY_LEDGER_SCHEMA,
                ...
                "mapping": {
                    "lower_percentile": recipe.lower_percentile,
                    "upper_percentile": recipe.upper_percentile,
        ...
        _write_json(
            ...
            {"schema": INTENSITY_VALIDATION_SCHEMA, **validation.to_dict()},
        )
        files = {
        ...
        _write_json(
            partial / result.manifest.name,
            {
                "schema": INTENSITY_MANIFEST_SCHEMA,
                "product_kind": "intensity_relief",
                "build_id": build_id,
                "identity": identity,
                "recipe_id": recipe.recipe_id,
                ...
                "source_provenance": {
                    ...
                    "master_product_id": field.master_product_id,
                    "master_array_sha256": field.master_array_sha256,
                    "kinematical_recipe_id": provenance["kinematical_recipe_id"],
                    "reflector_catalog_evidence": provenance["reflector_catalog_evidence"],
                    "sampling": provenance["sampling"],
                    "seam_diagnostics": provenance["seam_diagnostics"],
                    "projection_ledger": provenance["projection_ledger"],
