...
| Matching compatibility | Present for canonical signals: exact S2 grid, normalized cosine, and named input normalization. A Python-only same-source detector-to-partial-S2 geometry proof now records bilinear sampling and a coverage mask. | The proof uses a coverage-specific masked cosine score, not the current full-S2 `ebsdx-rs` metric. Detector dtype, mask, background/saturation policy, and an interoperable adapter recipe remain open. |
