...
| Runtime geometry | Explicitly not fixed: the package is detector-independent. Python now has finite candidate geometry ranking on a common S2 mask and a three-profile source-bound virtual-camera transfer proof. | `ebsdxr dictionary-resource-preflight --require-runtime --runtime-recipe <json>` now rejects absent geometry, sample-frame convention, or unnamed preprocessing; it never infers any of them. The Rust runtime has no detector-to-S2 or geometry-search execution yet. |
