...
- [ ] Exact oriented S2 nodes preserve source ordering, values, unit norms, and provenance.
- [ ] Identity and Bunge `(17, 31, 43)` degree rotations pass inverse and adapter-parity diagnostics.
- [ ] Upper/lower hemisphere reprojection uses an explicit inverse spherical mapping with no image rotation or blur.
- [ ] The bundle contains identity comparison, both hemispheres, two sphere views, axis diagnostics, and complete ledgers.
- [ ] A bounded smoke passes before one 2400-pixel Ice review candidate is produced.
