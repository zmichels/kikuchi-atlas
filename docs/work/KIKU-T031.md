...
- [x] The canonical forsterite recipe loads with the approved source hashes, `80.0 mm` base diameter, `1.2 mm` outward relief, subdivision `7`, global `1/99` percentiles, gamma `1.0`, and `0.8 mm` spherical Gaussian FWHM.
- [x] Unknown keys, malformed identities, booleans-as-numbers, non-finite values, and unsupported semantic choices fail closed with focused errors.
- [x] Recipe IDs are deterministic, path-independent, and covered by focused unit tests and Ruff.
